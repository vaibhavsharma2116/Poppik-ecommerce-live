...
export default function ThermalInvoice() {
  ...
  useEffect(() => {
    try {
      ...
      // Try to fetch order details if API available
      if (id) {
        fetch(`/api/orders/${id}`)
          ...
          .catch(() => {
            ...
            fetch(`/api/admin/orders?orderId=${id}`)
              .then(r => r.ok ? r.json() : Promise.reject())
              .then((data) => {
  ...
  const total = items.reduce((s, it) => s + (parseFloat(String(it.price).replace(/[₹,]/g, '')) || 0) * (it.quantity || 1), 0);

  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-3xl mx-auto bg-white p-6 rounded shadow">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-xl font-bold">Thermal Invoice</h1>
          <div>
            <button onClick={() => window.print()} className="px-3 py-1 bg-black text-white rounded">Print</button>
          </div>
        </div>

        <p className="mb-4"><strong>Order ID:</strong> {orderId || 'N/A'}</p>
            ...
            <tbody>
              {items.map((it, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <td className="py-2 text-right">{typeof it.price === 'number' ? `₹${it.price}` : it.price}</td>
                  <td className="py-2 text-right">₹{((parseFloat(String(it.price).replace(/[₹,]/g, '')) || 0) * (it.quantity || 1)).toFixed(2)}</td>
                </tr>
              ))}
              <tr>
                <td colSpan={3} className="py-3 text-right font-bold">Grand Total</td>
                <td className="py-3 text-right font-bold">₹{total.toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
        )}
