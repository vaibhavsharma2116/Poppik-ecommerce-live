...
export default function ProductDetail() {
  const [, rawParams] = useRoute("/product/:slug");
  ...
  const productSlug = useMemo(() => (params?.slug ?? null) as string | null, [params?.slug]);
  ...
  const { data: productImages = [] } = useQuery<any[]>({
  ...
  const imageUrls = useMemo(() => {
    ...
    if (productImages && productImages.length > 0) {
      ...
      const imageUrlsFromDb = productImages
        ...
        .filter((url: any): url is string => typeof url === 'string' && url.trim() !== ''); // Filter out empty/null URLs
  ...
  const { data: recommendedProductsData } = useQuery({
    queryKey: ['/api/products', { limit: 12 }],
  } as any);

  const recommendedProducts = (recommendedProductsData ?? []) as any[];
                          ...
                          <Dialog open={showShareDialog} onOpenChange={setShowShareDialog}>
                            ...
                            <DialogContent className="sm:max-w-md">
                              ...
                              <div className="space-y-3 py-4">
                                <Button variant="outline" className="w-full justify-start gap-3 h-12" onClick={shareToWhatsApp}>
                                  WhatsApp
                                </Button>
                                <Button variant="outline" className="w-full justify-start gap-3 h-12" onClick={shareToFacebook}>
                                  Facebook
                                </Button>
                                <Button variant="outline" className="w-full justify-start gap-3 h-12" onClick={shareToTwitter}>
                                  Twitter
                                </Button>
                              </div>
                            </DialogContent>
