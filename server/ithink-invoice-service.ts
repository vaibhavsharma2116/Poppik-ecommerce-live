...
export class IthinkInvoiceService {
  private ithink: IthinkService;

  constructor(ithink: IthinkService) {
    this.ithink = ithink;
  }

  /**
   * Streams delivery partner generated invoice PDF to the client.
   * - Never exposes delivery partner token or delivery partner PDF URL to the caller.
   */
  async streamThermalInvoicePdf(
    res: Response,
    ithinkOrderId: string | number,
    publicOrderId: string,
    awb: string | null,
    filename: string
  ) {
    void publicOrderId;
    const pdfResp = await this.ithink.generateInvoicePdfResponse(ithinkOrderId);
  ...
  /**
   * Streams delivery partner generated label PDF to the client.
   * - Never exposes delivery partner token or delivery partner PDF URL to the caller.
   */
  async streamThermalLabelPdf(res: Response, shipmentId: number, filename: string) {
    const pdfResp = await this.ithink.generateLabelPdfResponse(shipmentId);
